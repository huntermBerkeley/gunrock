...
class vector_frontier_t {
 public:
  ...
  vector_frontier_t()
    ...
    cudaMallocHost((void **)&new_pointer, sizeof(type_t)*1);
    //slower than memmove but easy

    internal_num_elems = 1;
  ...
  vector_frontier_t(std::size_t size, float frontier_resizing_factor = 1.0f)
    ...
    cudaMallocHost((void **)&new_pointer, sizeof(type_t)*size);

    internal_num_elems = size;
    this->set_number_of_elements(size);
  ...
  __device__ __host__ vector_frontier_t(const vector_frontier_t& rhs) {
    ...
    internal_num_elems = rhs.internal_num_elems;
   ...
   * @return std::size_t
   */
  std::size_t get_capacity() const { return internal_num_elems; }
   ...
   */
  __host__ __device__ __forceinline__ constexpr type_t * get() const {
    return raw_ptr;
  }
   ...
   */
  type_t * end() { return this->begin() + this->get_number_of_elements(); }
  ...
  void push_back(type_t const& value) {
    ...
    if (num_elements + 1 >= internal_num_elems){
      reserve(internal_num_elems*resizing_factor);
    }


    raw_ptr[num_elements] = value;
    num_elements++;
  }
  ...
  void resize(
    ...
    cudaMemcpy(new_pointer, raw_ptr, sizeof(type_t)*num_elements);

    //slower than memmove but easy
    for (uint64_t i = num_elements; i < size; i++){
      new_pointer[i] = default_value;
    }

    cudaFreeHost(raw_ptr);

    //p_storage.reset(new_pointer);

    internal_num_elems = num_elements;
    this->set_number_of_elements(num_elements);
  ...
  void reserve(std::size_t const& size) {
    ...
    cudaMallocHost((void **)&new_pointer, sizeof(type_t)*size*resizing_factor);

    memcpy(new_pointer, raw_ptr, sizeof(type_t)*num_elements);
  ...
  void sort(sort::order_t order = sort::order_t::ascending,
            gcuda::stream_t stream = 0) {
    sort::radix::sort_keys(this->begin(),
